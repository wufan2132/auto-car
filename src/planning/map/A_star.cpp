...
double Astar::calcG(MapPoint *temp_start, MapPoint *point) {
  double extraG =
      (abs(point->x - temp_start->x) + abs(point->y - temp_start->y)) == 1
          ...
          : kCost2;
  double parentG =
      point->parent == NULL ? 0 : point->parent->G;  //����ǳ�ʼ�ڵ㣬���丸�ڵ��ǿ�
...
double Astar::calcF(MapPoint *point) { return kG * point->G + kH * point->H; }
...
MapPoint *Astar::findPath(MapPoint &startPoint, MapPoint &endPoint,
                          bool isIgnoreCorner) {
  openList.push_back(new MapPoint(
      startPoint.x, startPoint.y));  //�������,��������һ���ڵ㣬�������
  while (!openList.empty()) {
    auto curPoint = getLeastFpoint();  //�ҵ�Fֵ��С�ĵ�

    // std::cout << "find_node:" << openList.size() << "  cur_distant:" <<
    // curPoint->H << std::endl;
...
std::list<MapPoint *> Astar::GetPath(MapPoint &startPoint, MapPoint &endPoint,
                                     bool isIgnoreCorner) {
  MapPoint *result = findPath(startPoint, endPoint, isIgnoreCorner);
  std::list<MapPoint *> path;
  if (result == NULL) AERROR << "Astar::GetPath: Can not find a feasible path!";
  //����·�������û�ҵ�·�������ؿ�����
  while (result) {
...
MapPoint *Astar::isInList(const std::list<MapPoint *> &list,
...
// maze[x][y] == 0: 可走
//其他值：不可走
bool Astar::isCanreach(const MapPoint *point, const MapPoint *target,
  ...
  if (target->x < 0 || target->x > maze.size() - 1 ||
      ...
      isInList(
          closeList,
          target))  //������뵱ǰ�ڵ��غϡ�������ͼ�����ϰ�������ڹر��б��У�����false
    return false;
  else {
...
std::vector<MapPoint *> Astar::getSurroundPoints(const MapPoint *point,
                                                 bool isIgnoreCorner) const {
