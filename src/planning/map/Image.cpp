...
bool Image::Readpgm(string fileName) {
  ...
  if ((filePointer = fopen(fileName.c_str(), "r")) == NULL) {
    AERROR << "Image::Readpgm: cannot open file" << fileName;
    fclose(filePointer);
    return 0;
  }
  ...
  if (line[0] == 'P' && (line[1] == '2')) {
    ...
    AERROR << "Image::Readpgm: incorrect file format";
    fclose(filePointer);
    return 0;
  }
  ...
  if (cols < 1 || rows < 1 || maximumValue < 0) {
    AERROR
        << "Image::Readpgm: invalid file specifications (cols/rows/max value)";
    fclose(filePointer);
    return 0;
  }
  ...
  if (binary) {
    for (i = 0; i < rows; i++) {
      numberRead += fread((void *)&(image[i][0]), sizeof(unsigned char), cols,
                          filePointer);
  ...
  if (numberRead < rows * cols) {
    AERROR << "Image::Readpgm: fewer pixels than rows*cols indicates";
  }
  // resize
  data_R.resize(rows);
  for (auto &vec : data_R) vec.resize(cols);
  data_G.resize(rows);
  for (auto &vec : data_G) vec.resize(cols);
  data_B.resize(rows);
  for (auto &vec : data_B) vec.resize(cols);
  data.resize(rows);
  for (auto &vec : data) vec.resize(cols);
  for (i = 0; i < rows; i++) {
    for (j = 0; j < cols; j++) {
...
bool Image::Read24BMP(string strFileName) {
  ...
  if (fread((void *)&FileHeader, 1, 14, fp) != 14) {
    fclose(fp);
    return 0;
  }
  if (fread((void *)&InfoHeader, 1, 40, fp) != 40) {
    fclose(fp);
    return 0;
  }
  ...
  if (cols * rows == 0) {
    AERROR << "Image::Read24BMP: Image is null!";
    ;
    return 0;
  }
  Patch = 3 * cols;
  ...
  if ((pBmpBuf = new BYTE[ImgSize]) == NULL) {
    fclose(fp);
    return 0;
  }

  Extend = (cols * 3 + 3) / 4 * 4 - cols * 3;
  for (pCur = pBmpBuf + ImgSize - Patch; pCur >= pBmpBuf; pCur -= Patch) {
...
bool Image::Write24BMP(string strFileName) {
  ...
  Extend = (cols * 3 + 3) / 4 * 4 - cols * 3;
  for (pCur = pImg + (rows - 1) * Patch; pCur >= pImg; pCur -= Patch) {
...
void Image::drawPoint(int x, int y, char color, int size) {
  for (int i = x - size / 2; i <= x + size / 2; i++)
    for (int j = y - size / 2; j <= y + size / 2; j++) setcolor(i, j, color);
}
  ...
  for (int i = 0; i < img.rows; i++) {
    for (int j = 0; j < img.cols; j++) {
      for (int k = 0; k < 3; k++) {
        ...
        after_j = j;
        pre_i = min(rows,
                    (int)round((double)after_i /
                               scale));  /////ȡ������ֵ����Ϊ�����ڽ���ֵ������ȡ������
        pre_j = min(cols, (int)round((double)after_j / scale));
        //��ԭͼ��Χ��
        *(img.pBmpBuf + i * img.cols * 3 + j * 3 + k) =
            *(pBmpBuf + pre_i * cols * 3 + pre_j * 3 + k);
      }
    }
  }
  //�������ؾ���
