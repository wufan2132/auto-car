...
class Astar {
 public:
  Astar(const common::base::ConfNode &yaml_conf);
  void InitAstar(std::vector<std::vector<int>> &_maze);
  std::list<MapPoint *> GetPath(MapPoint &startPoint, MapPoint &endPoint,
                                bool isIgnoreCorner);

  void setBMP(Image *pImg) { img = pImg; };
  Image *img;

  static void show_path(std::list<MapPoint *> &path, Image *img,
                        char color = 'r');

  static void smooth(std::list<MapPoint *> &path, int order);

 private:
  MapPoint *findPath(MapPoint &startPoint, MapPoint &endPoint,
                     bool isIgnoreCorner);
  std::vector<MapPoint *> getSurroundPoints(const MapPoint *MapPoint,
                                            bool isIgnoreCorner) const;
  bool isCanreach(const MapPoint *point, const MapPoint *target,
                  bool isIgnoreCorner) const;  //�ж�ĳ���Ƿ����������һ���ж�
  MapPoint *isInList(const std::list<MapPoint *> &list,
                     const MapPoint *point) const;  //�жϿ���/�ر��б����Ƿ����ĳ��
