...
AnalyticPolynomial* Interpolating::process(
  ...
  for (int i = 0; i < csp->length; i++) {
    ...
    //
    AINFO << point.x << ',' << point.y;
  }
  return csp;
}

void Interpolating::Spline2D(const VectorXf& point_x, const VectorXf& point_y,
                             AnalyticPolynomial& csp, float spacing) {
...
void Interpolating::spline(const VectorXf& x, const VectorXf& y, MatrixXf& sp) {
  int len = x.rows();
  VectorXf hs(len - 1);
  for (int i = 0; i < len - 1; i++) hs(i) = x(i + 1) - x(i);
  // cout << "hs" << endl;
  //����ϵ������A
  MatrixXf A = MatrixXf::Zero(len, len);
  A(0, 0) = 1;
  for (int i = 0; i < len - 1; i++) {
    if (i != len - 2) A(i + 1, i + 1) = 2 * (hs(i) + hs(i + 1));
    A(i + 1, i) = hs(i);
    A(i, i + 1) = hs(i);
  }
  A(0, 1) = 0;
  ...
  for (int i = 0; i < len - 2; i++)
    B(i + 1) =
        3 * (y(i + 2) - y(i + 1)) / hs(i + 1) - 3 * (y(i + 1) - y(i)) / hs(i);
  B(0) = 0;
  B(len - 1) = 0;
  // cout << "B"<<endl;
  /*��� Ac=B*/
...
void Interpolating::cal_position(const VectorXf& step, const MatrixXf& sx,
                                 const MatrixXf& sy, MatrixXf& xout,
                                 MatrixXf& yout) {
  int len = step.rows();
  int idx;
  float dx, dy;
  ...
  for (int i = 0; i < len; i++) {
    ...
    dx = step(i) - sx(0, idx);
    xout(0, i) = sx(2, idx) + sx(3, idx) * dx + sx(4, idx) * dx * dx +
                 sx(5, idx) * dx * dx * dx;
    xout(1, i) = sx(3, idx) + 2 * sx(4, idx) * dx + 3 * sx(5, idx) * dx * dx;
    xout(2, i) = 2 * sx(4, idx) + 6 * sx(5, idx) * dx;
    dy = step(i) - sy(0, idx);
    yout(0, i) = sy(2, idx) + sy(3, idx) * dy + sy(4, idx) * dy * dy +
                 sy(5, idx) * dy * dy * dy;
...
void Interpolating::cal_yaw(const MatrixXf& xout, const MatrixXf& yout,
                            VectorXf& yaw) {
  int len = xout.cols();
  yaw.resize(len);
  for (int i = 0; i < len; i++) {
...
void Interpolating::cal_curvature(const MatrixXf& xout, const MatrixXf& yout,
                                  VectorXf& curvature) {
  int len = xout.cols();
  curvature.resize(len);
  for (int i = 0; i < len; i++) {
    curvature(i) = (yout(2, i) * xout(1, i) - yout(1, i) * xout(2, i)) /
                   (xout(1, i) * xout(1, i) + yout(1, i) * yout(1, i));
  }
}
