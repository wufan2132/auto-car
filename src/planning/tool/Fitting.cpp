...
VectorXf Fitting::quartic4_polynomial(float now_s, float now_speed,
...
VectorXf Fitting::quintic5_polynomial(float now_hs, float now_hs_d,
  ...
  B << aim_hs - now_hs - now_hs_d * t - now_hs_dd * t * t / 2,
      aim_hs_d - now_hs_d - now_hs_dd * t, aim_hs_dd - now_hs_dd;
  VectorXf QP(6);
...
void Fitting::cal_point_quartic4(const VectorXf& QP, const VectorXf& t,
                                 MatrixXf& St) {
  int len = t.rows();
  St.resize(len, 4);
  for (int i = 0; i < len; i++) {
    St.row(i) << QP(0) + QP(1) * t(i) + QP(2) * t(i) * t(i) +
                     QP(3) * t(i) * t(i) * t(i) +
                     QP(4) * t(i) * t(i) * t(i) * t(i),
        QP(1) + 2 * QP(2) * t(i) + 3 * QP(3) * t(i) * t(i) +
            4 * QP(4) * t(i) * t(i) * t(i),
...
void Fitting::cal_point_quartic4(const VectorXf& QP, double t, VectorXf& St) {
  ...
  St << QP(0) + QP(1) * t + QP(2) * t * t + QP(3) * t * t * t +
            QP(4) * t * t * t * t,
...
double Fitting::cal_point_quartic4(const VectorXf& QP, double t) {
  return QP(0) + QP(1) * t + QP(2) * t * t + QP(3) * t * t * t +
         QP(4) * t * t * t * t;
}

void Fitting::cal_point_quintic5(const VectorXf& QP, const VectorXf& t,
                                 MatrixXf& Xt) {
  int len = t.rows();
  Xt.resize(len, 4);
  for (int i = 0; i < len; i++) {
    Xt.row(i) << QP(0) + QP(1) * t(i) + QP(2) * t(i) * t(i) +
                     QP(3) * t(i) * t(i) * t(i) +
                     QP(4) * t(i) * t(i) * t(i) * t(i) +
                     QP(5) * t(i) * t(i) * t(i) * t(i) * t(i),
        QP(1) + 2 * QP(2) * t(i) + 3 * QP(3) * t(i) * t(i) +
            4 * QP(4) * t(i) * t(i) * t(i) +
            5 * QP(5) * t(i) * t(i) * t(i) * t(i),
        2 * QP(2) + 6 * QP(3) * t(i) + 12 * QP(4) * t(i) * t(i) +
            20 * QP(5) * t(i) * t(i) * t(i),
...
void Fitting::cal_point_quintic5(const VectorXf& QP, double t, VectorXf& Xt) {
  Xt.resize(4);
  Xt << QP(0) + QP(1) * t + QP(2) * t * t + QP(3) * t * t * t +
            QP(4) * t * t * t * t + QP(5) * t * t * t * t * t,
      QP(1) + 2 * QP(2) * t + 3 * QP(3) * t * t + 4 * QP(4) * t * t * t +
          5 * QP(5) * t * t * t * t,
